...
export function resolveEnv(key: string, runtimeEnv?: Record<string, string | undefined>): string {
  ...
  // 3. Vite SSR dev — .env vars are only available through import.meta.env, not process.env
  //    Dynamic bracket access works in SSR dev (real object), but not in production build (static replacement).
  //    This is fine because production uses runtimeEnv from cloudflare:workers (step 1).
  try {
